...
class RolePermissions
{
    ...
    public function filterAdminAccess(): void
    {
        ...
        $page = $_GET['page'] ?? '';
        if (strpos($page, 'ai-seo-assistant') !== 0) {
            return;
        }
        ...
        if ($requiredCap && !current_user_can($requiredCap)) {
            wp_die(esc_html__('You do not have permission to access this page.', 'ai-seo-assistant'), 403);
        }
    }
    ...
    {
        $map = [
            'ai-seo-assistant' => 'aiseo_manage_settings',
    ...
    public function renderPermissionsSettings(): void
    {
        ...
        ?>
        <h3><?php esc_html_e('Role-Based Permissions', 'ai-seo-assistant'); ?></h3>
        <p><?php esc_html_e('Configure which user roles can access AI SEO features:', 'ai-seo-assistant'); ?></p>
        ...
        <p class="description">
            <?php esc_html_e('Note: Administrators have all permissions. Permissions can be customized with a roles plugin.', 'ai-seo-assistant'); ?>
        </p>
        <?php
    }
