...
class TenantRepository
{
    ...
    public function createTenant(array $data): array|\WP_Error
    {
        ...
        if (empty($data['name']) || empty($data['email'])) {
            return new \WP_Error('missing_fields', __('Name and email are required', 'ai-seo-assistant'));
        }
        ...
        if ($existing) {
            return new \WP_Error('duplicate_key', __('Tenant key already exists', 'ai-seo-assistant'));
        }
        ...
        if ($result === false) {
            return new \WP_Error('db_error', __('Failed to create tenant', 'ai-seo-assistant'));
        }
    ...
    public function updateTenant(string $tenantKey, array $data): bool|\WP_Error
    {
        ...
        if (empty($update)) {
            return new \WP_Error('no_data', __('No data to update', 'ai-seo-assistant'));
        }
