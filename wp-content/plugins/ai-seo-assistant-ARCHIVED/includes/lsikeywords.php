...
class LSIKeywords
{
    ...
    public function addMetaBoxes(): void
    {
        ...
        foreach ($screens as $screen) {
            add_meta_box(
                'aiseo_lsi',
                __('LSI Keywords & Synonyms', 'ai-seo-assistant'),
                [$this, 'renderMetaBox'],
                $screen,
                'normal',
                'default'
    ...
    public function renderMetaBox(\WP_Post $post): void
    {
        ...
        <div class="aiseo-lsi-box">
            <p>
                <strong><?php esc_html_e('Focus Keyphrase:', 'ai-seo-assistant'); ?></strong>
                <?php echo $focusKeyphrase ? esc_html($focusKeyphrase) : '<em>' . esc_html__('(Set in SEO tab)', 'ai-seo-assistant') . '</em>'; ?>
            </p>
            
            <?php if ($focusKeyphrase) : ?>
            <p>
                <button type="button" class="button" id="aiseo-generate-lsi" data-post="<?php echo $post->ID; ?>">
                    <?php esc_html_e('Generate LSI Keywords', 'ai-seo-assistant'); ?>
                </button>
            ...
            <div id="aiseo-lsi-suggestions" style="margin-top:15px;">
                <?php if ($suggested) : ?>
                <h4><?php esc_html_e('Suggested Keywords:', 'ai-seo-assistant'); ?></h4>
                ...
                <?php else : ?>
                <p><em><?php esc_html_e('Click "Generate LSI Keywords" to get suggestions based on your focus keyphrase.', 'ai-seo-assistant'); ?></em></p>
                <?php endif; ?>
            </div>
            
            <p style="margin-top:15px;">
                <strong><?php esc_html_e('Coverage:', 'ai-seo-assistant'); ?></strong>
                <span id="aiseo-lsi-coverage">
                    <?php 
                    $coverage = $this->calculateCoverage($post->ID, $focusKeyphrase, $used);
                    printf(__('%.0f%% of LSI keywords used in content', 'ai-seo-assistant'), $coverage);
                    ?>
                </span>
            </p>
            <?php else : ?>
            <p class="notice notice-warning">
                <?php esc_html_e('Please set a focus keyphrase in the SEO Score tab first to generate LSI keywords.', 'ai-seo-assistant'); ?>
            </p>
            <?php endif; ?>
        </div>
        ...
        jQuery(document).ready(function($) {
            $('#aiseo-generate-lsi').on('click', function() {
                ...
                wp.apiFetch({
                    ...
                    var html = '<h4><?php echo esc_js(__('Suggested Keywords:', 'ai-seo-assistant')); ?></h4>';
...
Output as a comma-separated list only.";
    ...
    public function restSuggestLSI(\WP_REST_Request $request): array
    {
        ...
        if (!$focusKeyphrase) {
            return new \WP_Error('no_keyphrase', __('No focus keyphrase set', 'ai-seo-assistant'));
        }
