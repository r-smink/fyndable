...
class ImageAltGenerator
{
    ...
    public function addMetaBoxes(): void
    {
        add_meta_box(
            'aiseo_image_alt',
            __('AI Alt Text Generator', 'ai-seo-assistant'),
            [$this, 'renderMetaBox'],
            'attachment',
            'side',
            'high'
    ...
    public function renderMetaBox(\WP_Post $post): void
    {
        ...
        <div class="aiseo-image-alt-box">
            <p>
                <label><strong><?php esc_html_e('Current Alt Text', 'ai-seo-assistant'); ?></strong></label><br>
                <input type="text" value="<?php echo esc_attr($currentAlt); ?>" class="wide-text" readonly>
            </p>
            
            <?php if ($generatedAlt) : ?>
            <p>
                <label><strong><?php esc_html_e('AI Suggested', 'ai-seo-assistant'); ?></strong></label><br>
            ...
            <p>
                <button type="button" class="button" id="aiseo-apply-alt">
                    <?php esc_html_e('Apply AI Suggestion', 'ai-seo-assistant'); ?>
                </button>
            </p>
            <?php endif; ?>
            ...
            <p>
                <button type="button" class="button button-primary" id="aiseo-generate-alt" data-attachment="<?php echo $post->ID; ?>">
                    <?php esc_html_e('Generate AI Alt Text', 'ai-seo-assistant'); ?>
                </button>
            </p>
            ...
            <div id="aiseo-alt-result" style="margin-top:10px; display:none;">
                ...
                <p>
                    <button type="button" class="button" id="aiseo-use-alt"><?php esc_html_e('Use This', 'ai-seo-assistant'); ?></button>
                </p>
            </div>
        </div>
        ...
        jQuery(document).ready(function($) {
            $('#aiseo-generate-alt').on('click', function() {
                ...
                btn.prop('disabled', true).text('<?php esc_html_e('Analyzing...', 'ai-seo-assistant'); ?>');
                ...
                wp.apiFetch({
                    ...
                    btn.prop('disabled', false).text('<?php esc_html_e('Generate AI Alt Text', 'ai-seo-assistant'); ?>');
    ...
    public function renderAttachmentMeta(): void
    {
        ...
        <div class="misc-pub-section">
            <strong><?php esc_html_e('AI Alt Text Suggestion', 'ai-seo-assistant'); ?></strong><br>
            ...
            <button type="button" class="button-link" onclick="jQuery('#attachment_alt').val('<?php echo esc_js($generatedAlt); ?>');">
                <?php esc_html_e('Use suggestion', 'ai-seo-assistant'); ?>
            </button>
        </div>
        <?php
    }
    ...
    {
        if (!wp_attachment_is_image($attachmentId)) {
            return new \WP_Error('not_image', __('Attachment is not an image', 'ai-seo-assistant'));
        }

        // Get image file path
        $filePath = get_attached_file($attachmentId);
        if (!$filePath || !file_exists($filePath)) {
            return new \WP_Error('no_file', __('Image file not found', 'ai-seo-assistant'));
        }
