...
class TenantRepository
{
     ...
     *
     * Blocked when the tenant has sub-tenants or generated sub-licenses.
     * Invoices (sseo_ai_invoices) are intentionally kept for bookkeeping.
     *
     * @param string $tenantKey
     * @param string $licenseAction What to do with the linked license:
     *                              keep|free (reusable)|revoke|delete
     */
    public function deleteTenant(string $tenantKey, string $licenseAction = 'keep'): bool|\WP_Error
    {
        global $wpdb;
        $table = $wpdb->prefix . self::TENANTS_TABLE;
        ...
        if (!$tenant) {
            return new \WP_Error('not_found', __('Tenant not found', 'sseo-ai-saas'));
        }

        $tenantId = (int)$tenant['id'];

        // Block deletion for agency tenants that still have sub-tenants
        $subCount = $this->countSubTenants($tenantId);
        if ($subCount > 0) {
            return new \WP_Error(
                'has_subtenants',
                sprintf(
                    __('Cannot delete tenant: %d sub-tenant(s) are still linked. Delete or move them first.', 'sseo-ai-saas'),
                    $subCount
                )
            );
        }

        // Block deletion when sub-licenses generated by this agency tenant exist
        $licenseTable = $wpdb->prefix . self::LICENSE_KEYS_TABLE;
        $subLicenses = (int)$wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $licenseTable WHERE agency_tenant_id = %d",
            $tenantId
        ));
        if ($subLicenses > 0) {
            return new \WP_Error(
                'has_sub_licenses',
                sprintf(
                    __('Cannot delete tenant: %d sub-license(s) were generated by this agency. Revoke or delete them first.', 'sseo-ai-saas'),
                    $subLicenses
                )
            );
        }

        // Handle the linked license key
                ...
                case 'revoke':
                    $wpdb->update(
                        ...
                        [
                            ...
                            'revoked_reason' => __('Tenant deleted by admin', 'sseo-ai-saas'),
                        ],
                        ['license_key' => $licenseKey]
                    );
                    break;
        ...
        $ticketsTable = $wpdb->prefix . self::SUPPORT_TICKETS_TABLE;
        $repliesTable = $wpdb->prefix . self::SUPPORT_REPLIES_TABLE;
        $ticketIds = $wpdb->get_col($wpdb->prepare(
            "SELECT id FROM $ticketsTable WHERE tenant_id = %d",
            $tenantId
        ));
        if (!empty($ticketIds)) {
            $placeholders = implode(',', array_fill(0, count($ticketIds), '%d'));
            $wpdb->query($wpdb->prepare(
                "DELETE FROM $repliesTable WHERE ticket_id IN ($placeholders)",
                ...$ticketIds
            ));
            $wpdb->delete($ticketsTable, ['tenant_id' => $tenantId]);
        }

        // Other per-tenant data keyed by numeric tenant id
        foreach ([
            self::TENANT_SETTINGS_TABLE,
            self::TENANT_USAGE_TABLE,
            self::GOOGLE_API_USAGE_TABLE,
            'sseo_ai_feedback',
            self::AGENCY_ACCOUNTS_TABLE,
        ] as $relatedTable) {
            $fullTable = $wpdb->prefix . $relatedTable;
            if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $fullTable))) {
                $wpdb->delete($fullTable, ['tenant_id' => $tenantId]);
            }
        }

        // Legacy data tables store tenant_id as the tenant_key string
        foreach (['sseo_ai_snapshots', 'sseo_ai_ai_overviews', 'sseo_ai_content_decay', 'sseo_ai_position_trends'] as $legacyTable) {
            $fullTable = $wpdb->prefix . $legacyTable;
            if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $fullTable))) {
                $wpdb->delete($fullTable, ['tenant_id' => $tenantKey]);
            }
        }

        return $wpdb->delete($table, ['tenant_key' => $tenantKey]) !== false;
    }
