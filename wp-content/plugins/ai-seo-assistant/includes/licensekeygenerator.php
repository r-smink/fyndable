...
class LicenseKeyGenerator
{
    ...
    public function generateLicense(array $options = []): array|\WP_Error
    {
        ...
        if ($result === false) {
            return new \WP_Error('db_error', __('Failed to generate license key', 'ai-seo-assistant'));
        }
    ...
    public function batchGenerateLicenses(int $count, array $options = []): array|\WP_Error
    {
        if ($count < 1 || $count > 100) {
            return new \WP_Error('invalid_count', __('Count must be between 1 and 100', 'ai-seo-assistant'));
        }
        ...
        if (empty($licenses)) {
            return new \WP_Error('batch_failed', __('Failed to generate any licenses', 'ai-seo-assistant'));
        }
    ...
    public function activateLicense(string $licenseKey, array $activationData = []): array|\WP_Error
    {
        ...
        if (!$license) {
            return new \WP_Error('invalid_key', __('License key not found', 'ai-seo-assistant'));
        }
        
        // Check status
        if ($license['status'] === 'revoked') {
            return new \WP_Error('revoked', __('This license key has been revoked', 'ai-seo-assistant'));
        }
        
        if ($license['status'] === 'expired') {
            return new \WP_Error('expired', __('This license key has expired', 'ai-seo-assistant'));
        }
        ...
        $siteHash = $this->getSiteHash($activationData['site_url'] ?? '');
    ...
    public function validateLicense(string $licenseKey): array|\WP_Error
    {
        ...
        if (!empty($license['expires_at']) && strtotime($license['expires_at']) < time()) {
            return new \WP_Error('expired', __('License key has expired', 'ai-seo-assistant'));
        }
        
        // Check if revoked
        if ($license['status'] === 'revoked') {
            return new \WP_Error('revoked', __('License key has been revoked', 'ai-seo-assistant'));
        }
    ...
    public function updateLicense(string $licenseKey, array $data): bool|\WP_Error
    {
        ...
        if (empty($update)) {
            return new \WP_Error('no_data', __('No valid fields to update', 'ai-seo-assistant'));
        }
    ...
    /**
     * Get hash of site URL for tracking
     */
    private function getSiteHash(string $url): string
    {
