...
class ContentWriter
{
    ...
    public function addMenu(): void
    {
        add_submenu_page(
            'ai-seo-assistant',
            __('Content Writer', 'ai-seo-assistant'),
            __('Content Writer', 'ai-seo-assistant'),
            'manage_options',
    ...
    public function writeArticle(string $keyword, array $options = []): array|\WP_Error
    {
        ...
        $article = $intro . "\n\n" . implode("\n\n", $sections);
        if ($faq) {
            $article .= "\n\n<h2>" . __('Frequently Asked Questions', 'ai-seo-assistant') . "</h2>\n\n" . $faq;
        }
        $article .= "\n\n<h2>" . __('Conclusion', 'ai-seo-assistant') . "</h2>\n\n" . $conclusion;
...
Return ONLY the meta description, nothing else.";
    ...
    private function buildSections(string $keyword, string $title, string $outline, ?array $brief, string $tone, int $targetWords): array|\WP_Error
    {
        ...
        if (empty($sections)) {
            return new \WP_Error('no_sections', __('Failed to generate any content sections', 'ai-seo-assistant'));
        }
...
- Return ONLY the headings, one per line, prefixed with 'H2: '";
    ...
    public function restWriteSection(\WP_REST_Request $request): array|\WP_Error
    {
        ...
        if (!$keyword || !$heading) {
            return new \WP_Error('missing_params', __('Keyword and heading are required', 'ai-seo-assistant'));
        }
    ...
    public function renderPage(): void
    {
        ...
        <div class="wrap aiseo-modern">
            <h1><?php esc_html_e('AI Content Writer', 'ai-seo-assistant'); ?></h1>
            <p class="description"><?php esc_html_e('Generate full SEO-optimized articles with AI. Optionally uses Content Brief data for data-driven writing.', 'ai-seo-assistant'); ?></p>

            <div style="max-width: 900px;">
                <div class="postbox" style="padding: 20px;">
                    <h2><?php esc_html_e('Article Settings', 'ai-seo-assistant'); ?></h2>
                    
                    <table class="form-table">
                        <tr>
                            <th><label for="writer-keyword"><?php esc_html_e('Target Keyword', 'ai-seo-assistant'); ?></label></th>
                            <td><input type="text" id="writer-keyword" class="regular-text" style="width:100%;" placeholder="<?php esc_attr_e('Enter your main keyword...', 'ai-seo-assistant'); ?>"></td>
                        </tr>
                        <tr>
                            <th><label for="writer-title"><?php esc_html_e('Title (optional)', 'ai-seo-assistant'); ?></label></th>
                            <td><input type="text" id="writer-title" class="regular-text" style="width:100%;" placeholder="<?php esc_attr_e('Leave empty for AI-generated title', 'ai-seo-assistant'); ?>"></td>
                        </tr>
                        <tr>
                            <th><label for="writer-tone"><?php esc_html_e('Tone', 'ai-seo-assistant'); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="writer-words"><?php esc_html_e('Target Word Count', 'ai-seo-assistant'); ?></label></th>
                            <td>
                                <input type="range" id="writer-words" min="500" max="5000" step="100" value="1500" style="width:300px;">
                                <span id="writer-words-display">1500</span> <?php esc_html_e('words', 'ai-seo-assistant'); ?>
                            </td>
                        </tr>
                        <tr>
                            <th><label for="writer-outline"><?php esc_html_e('Custom Outline (optional)', 'ai-seo-assistant'); ?></label></th>
                            <td>
                                <textarea id="writer-outline" rows="5" class="large-text" placeholder="<?php esc_attr_e("H2: First section\nH2: Second section\nH3: Subsection\n...", 'ai-seo-assistant'); ?>"></textarea>
                                <p class="description"><?php esc_html_e('Leave empty to auto-generate from keyword analysis.', 'ai-seo-assistant'); ?></p>
                            </td>
                        </tr>
                        <tr>
                            <th><label><?php esc_html_e('Options', 'ai-seo-assistant'); ?></label></th>
                            <td>
                                <label>
                                    <input type="checkbox" id="writer-create-draft" checked> 
                                    <?php esc_html_e('Create WordPress draft automatically', 'ai-seo-assistant'); ?>
                                </label>
                            </td>
                        </tr>
                    </table>
                    ...
                    <p>
                        <button type="button" class="button button-primary button-hero" id="writer-generate">
                            <?php esc_html_e('Generate Article', 'ai-seo-assistant'); ?>
                        </button>
                    ...
                    <div class="postbox" style="padding: 20px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                            ...
                            <div>
                                ...
                                <a id="writer-edit-link" href="#" class="button" target="_blank" style="margin-left:10px;"><?php esc_html_e('Edit Draft', 'ai-seo-assistant'); ?></a>
                            </div>
                        </div>
                        ...
                        <p style="margin-top: 10px;">
                            <strong><?php esc_html_e('Meta Description:', 'ai-seo-assistant'); ?></strong>
        ...
        jQuery(document).ready(function($) {
            ...
            $('#writer-generate').on('click', function() {
                ...
                status.show().text('<?php echo esc_js(__('Analyzing keyword and generating article... This may take 1-2 minutes.', 'ai-seo-assistant')); ?>');
                $('#writer-result').hide();
                ...
                wp.apiFetch({
                    ...
                    status.text('<?php echo esc_js(__('Article generated successfully!', 'ai-seo-assistant')); ?>');
                }).catch(function(err) {
