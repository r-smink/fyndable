...
class TruSEOSCORE
{
    ...
    public function addMetaBoxes(): void
    {
        ...
        foreach ($screens as $screen) {
            add_meta_box(
                'aiseo_truseo',
                __('AI SEO Score & Optimization', 'ai-seo-assistant'),
                [$this, 'renderMetaBox'],
                $screen,
                'normal',
                'high'
    ...
    public function renderMetaBox(\WP_Post $post): void
    {
        ...
        <div class="aiseo-truseo-metabox">
            ...
            <div style="display:flex; gap:20px; margin-bottom:20px;">
                ...
                <div>
                    <h3><?php esc_html_e('SEO Score', 'ai-seo-assistant'); ?></h3>
                    <p><?php echo $score >= 80 ? __('Great! Your content is well-optimized.', 'ai-seo-assistant') : ($score >= 50 ? __('Your content needs some improvements.', 'ai-seo-assistant') : __('Major improvements needed.', 'ai-seo-assistant')); ?></p>
                </div>
            </div>

            <h4><?php esc_html_e('Focus Keyphrase', 'ai-seo-assistant'); ?></h4>
            <p>
                <input type="text" name="aiseo_focus_keyphrase" value="<?php echo esc_attr($focusKeyphrase); ?>" class="large-text" placeholder="<?php esc_attr_e('Enter your main keyword...', 'ai-seo-assistant'); ?>">
            </p>

            <h4><?php esc_html_e('Secondary Keyphrases', 'ai-seo-assistant'); ?></h4>
            <p>
                <input type="text" name="aiseo_secondary_keyphrases" value="<?php echo esc_attr($secondaryKeyphrases); ?>" class="large-text" placeholder="<?php esc_attr_e('keyword 1, keyword 2, keyword 3', 'ai-seo-assistant'); ?>">
                <small><?php esc_html_e('Comma-separated list of related keywords', 'ai-seo-assistant'); ?></small>
            </p>

            <h4><?php esc_html_e('SEO Title', 'ai-seo-assistant'); ?></h4>
            <p>
                <input type="text" name="aiseo_title" value="<?php echo esc_attr($title); ?>" class="large-text" maxlength="60">
                <small><?php printf(__('Recommended: 50-60 characters. Current: %d', 'ai-seo-assistant'), strlen($title)); ?></small>
            </p>

            <h4><?php esc_html_e('Meta Description', 'ai-seo-assistant'); ?></h4>
            <p>
                <textarea name="aiseo_description" rows="3" class="large-text" maxlength="160"><?php echo esc_textarea($description); ?></textarea>
                <small><?php printf(__('Recommended: 150-160 characters. Current: %d', 'ai-seo-assistant'), strlen($description)); ?></small>
            </p>

            <h4><?php esc_html_e('Google SERP Preview', 'ai-seo-assistant'); ?></h4>
            ...
            <h4><?php esc_html_e('Analysis Results', 'ai-seo-assistant'); ?></h4>
            ...
            <p>
                <button type="button" class="button" id="aiseo-analyze-content">
                    <?php esc_html_e('Re-analyze Content', 'ai-seo-assistant'); ?>
                </button>
                <button type="button" class="button button-secondary" id="aiseo-suggest-improvements">
                    <?php esc_html_e('AI Suggest Improvements', 'ai-seo-assistant'); ?>
                </button>
            </p>
    ...
    public function getAnalysis(\WP_Post $post, string $focusKeyphrase): array
    {
        ...
        $analysis[] = [
            'label' => __('Content Length', 'ai-seo-assistant'),
            'status' => $wordCount >= 500 ? 'good' : ($wordCount >= 300 ? 'warning' : 'error'),
            'message' => sprintf(__('%d words. %s', 'ai-seo-assistant'), $wordCount, 
                $wordCount >= 500 ? __('Good length.', 'ai-seo-assistant') : __('Try to add more content.', 'ai-seo-assistant')),
        ];
        ...
        if ($focusKeyphrase) {
            ...
            $analysis[] = [
                'label' => __('Keyphrase in Title', 'ai-seo-assistant'),
                'status' => $inTitle ? 'good' : 'error',
                'message' => $inTitle ? __('Keyphrase found in title.', 'ai-seo-assistant') : __('Add keyphrase to title.', 'ai-seo-assistant'),
            ];
            ...
            $density = $count / max(1, $wordCount) * 100;
            $analysis[] = [
                'label' => __('Keyphrase Density', 'ai-seo-assistant'),
                'status' => ($density >= 0.5 && $density <= 2.5) ? 'good' : 'warning',
                'message' => sprintf(__('%.1f%% (%d occurrences)', 'ai-seo-assistant'), $density, $count),
            ];
            ...
            $analysis[] = [
                'label' => __('Keyphrase in Intro', 'ai-seo-assistant'),
                'status' => $inIntro ? 'good' : 'warning',
                'message' => $inIntro ? __('Keyphrase appears in first paragraph.', 'ai-seo-assistant') : __('Add keyphrase to first paragraph.', 'ai-seo-assistant'),
            ];
        }
        ...
        $analysis[] = [
            'label' => __('Internal Links', 'ai-seo-assistant'),
            'status' => $internalCount >= 2 ? 'good' : 'warning',
            'message' => sprintf(__('%d internal links.', 'ai-seo-assistant'), $internalCount),
        ];

        // Images
        $images = $this->analyzeImages($post->post_content, $post->ID);
        $analysis[] = [
            'label' => __('Images with Alt Text', 'ai-seo-assistant'),
            'status' => ($images['total'] === 0 || $images['with_alt'] === $images['total']) ? 'good' : 'warning',
            'message' => sprintf(__('%d of %d images have alt text.', 'ai-seo-assistant'), $images['with_alt'], $images['total']),
        ];

        // H1 usage
        $h1Count = preg_match_all('/<h1[^>]*>/i', $post->post_content);
        $analysis[] = [
            'label' => __('H1 Heading', 'ai-seo-assistant'),
            'status' => $h1Count === 1 ? 'good' : 'error',
            'message' => $h1Count === 1 ? __('One H1 heading found.', 'ai-seo-assistant') : sprintf(__('%d H1 headings. Use exactly one.', 'ai-seo-assistant'), $h1Count),
        ];

        // Readability
        $readability = $this->calculateReadability($content);
        $analysis[] = [
            'label' => __('Readability', 'ai-seo-assistant'),
            'status' => $readability['flesch'] >= 50 ? 'good' : 'warning',
            'message' => sprintf(__('Flesch score: %d (%s)', 'ai-seo-assistant'), $readability['flesch'], $readability['grade']),
        ];
