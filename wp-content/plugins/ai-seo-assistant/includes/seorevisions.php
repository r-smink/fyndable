...
class SEORevisions
{
    ...
    public function addMetaBox(): void
    {
        add_meta_box(
            'aiseo_revisions',
            __('SEO Revisions', 'ai-seo-assistant'),
            [$this, 'renderMetaBox'],
            ['post', 'page', 'product'],
            'normal',
            'low'
    ...
    public function renderMetaBox(\WP_Post $post): void
    {
        ...
        <div class="aiseo-revisions-box">
            ...
            <table class="widefat striped">
                <thead>
                    <tr>
                        <th><?php esc_html_e('Date', 'ai-seo-assistant'); ?></th>
                        <th><?php esc_html_e('User', 'ai-seo-assistant'); ?></th>
                        <th><?php esc_html_e('SEO Score', 'ai-seo-assistant'); ?></th>
                ...
                <tbody>
                    ...
                    ?>
                    <tr>
                        <td><?php echo esc_html(human_time_diff(strtotime($rev['created_at']), current_time('timestamp'))); ?> <?php esc_html_e('ago', 'ai-seo-assistant'); ?></td>
                        ...
                        <td>
                            <button type="button" class="button button-small aiseo-restore-rev" data-rev="<?php echo $rev['id']; ?>">
                                <?php esc_html_e('Restore', 'ai-seo-assistant'); ?>
                            </button>
                        </td>
                    </tr>
                    <?php endforeach; ?>
            ...
            <?php else : ?>
            <p><em><?php esc_html_e('No SEO revisions yet.', 'ai-seo-assistant'); ?></em></p>
            <?php endif; ?>
        </div>
        ...
        jQuery(document).ready(function($) {
            $('.aiseo-restore-rev').on('click', function() {
                ...
                if (!confirm('<?php esc_html_e('Restore this SEO revision? Current SEO data will be overwritten.', 'ai-seo-assistant'); ?>')) {
                    return;
                }
                ...
                wp.apiFetch({
                    ...
                    alert('<?php esc_html_e('Revision restored. Reload the page to see changes.', 'ai-seo-assistant'); ?>');
                });
            });
        });
        </script>
    ...
    public function restRestoreRevision(\WP_REST_Request $request): array
    {
        ...
        return new \WP_Error('restore_failed', __('Failed to restore revision', 'ai-seo-assistant'));
    }
}
