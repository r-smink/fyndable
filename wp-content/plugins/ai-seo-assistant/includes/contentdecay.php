...
class ContentDecay
{
    ...
    private function generateSuggestions(\WP_Post $post, string $keyword, int $positionChange): array
    {
        ...
        if ($positionChange >= 5) {
            $suggestions[] = [
                ...
                'title' => __('Update content with fresh information', 'ai-seo-assistant'),
                'description' => __('Your content may be outdated. Consider adding new insights, statistics, or recent developments.', 'ai-seo-assistant'),
                'action' => 'edit',
        ...
        if ($positionChange >= 3) {
            $suggestions[] = [
                'type' => 'title_meta',
                'title' => __('Optimize title and meta description', 'ai-seo-assistant'),
                'description' => __('Review your title tag and meta description for better CTR.', 'ai-seo-assistant'),
                'action' => 'edit',
                'link' => get_edit_post_link($post->ID),
            ];
        }
        ...
        if ($internalLinks < 3) {
            $suggestions[] = [
                'type' => 'internal_links',
                'title' => __('Add more internal links', 'ai-seo-assistant'),
                'description' => sprintf(__('This post only has %d internal links. Add more to improve authority flow.', 'ai-seo-assistant'), $internalLinks),
        ...
        $suggestions[] = [
            ...
            'title' => __('Analyze top competitors', 'ai-seo-assistant'),
            'description' => __('See what content is now ranking higher and identify gaps.', 'ai-seo-assistant'),
    ...
    public function showDecayAlerts(): void
    {
        ...
        if (!empty($criticalAlerts)) {
            ...
            echo '<p><strong>' . esc_html__('Content Decay Alert:', 'ai-seo-assistant') . '</strong></p>';
            echo '<ul>';
            foreach ($criticalAlerts as $alert) {
                ...
                if ($post) {
                    echo '<li>';
                    echo sprintf(
                        esc_html__('"%1$s" dropped %2$d positions for "%3$s"', 'ai-seo-assistant'),
                        esc_html($post->post_title),
                        $alert['position_change'],
                        esc_html($alert['keyword'])
                    );
                    echo ' <a href="' . esc_url(get_edit_post_link($post->ID)) . '">' . esc_html__('Fix now', 'ai-seo-assistant') . '</a>';
                    echo '</li>';
                }
            }
            echo '</ul>';
            echo '<p><a href="' . esc_url(admin_url('admin.php?page=ai-seo-assistant-decay')) . '" class="button">' . esc_html__('View All Decay Alerts', 'ai-seo-assistant') . '</a></p>';
            echo '</div>';
        }
        
        if (!empty($highAlerts)) {
            echo '<div class="notice notice-warning is-dismissible">';
            echo '<p><strong>' . esc_html__('Content Decay Warning:', 'ai-seo-assistant') . '</strong> ';
            echo sprintf(esc_html_n('%d post is losing rankings', '%d posts are losing rankings', count($highAlerts), 'ai-seo-assistant'), count($highAlerts));
            echo '</p>';
            echo '</div>';
        }
    }
    ...
    public function addDecayMetaBox(): void
    {
        ...
        foreach ($postTypes as $postType) {
            add_meta_box(
                'aiseo_content_decay',
                __('SEO Position Trends', 'ai-seo-assistant'),
    ...
    public function renderDecayMetaBox(\WP_Post $post): void
    {
        ...
        if (!empty($alerts)) {
            ...
            echo '<p><strong>' . esc_html__('⚠️ Content Decay Detected', 'ai-seo-assistant') . '</strong></p>';
            foreach ($alerts as $alert) {
                echo '<p>';
                echo sprintf(
                    esc_html__('"%s" dropped %d positions', 'ai-seo-assistant'),
                    esc_html($alert['keyword']),
                    $alert['position_change']
                );
                echo '</p>';
        ...
        if (!empty($trends)) {
            ...
            echo '</div>';
            echo '<p class="description">' . esc_html__('Position trend (last 30 days)', 'ai-seo-assistant') . '</p>';
            ...
            echo '<p class="description">' . esc_html__('No trend data available yet. Track keywords to see position trends.', 'ai-seo-assistant') . '</p>';
        }
    ...
    public function renderDecayPage(): void
    {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Content Decay Detection', 'ai-seo-assistant'); ?></h1>
            ...
            <div class="tablenav top">
                <div class="alignleft actions">
                    <select id="filter-severity">
                        <option value=""><?php esc_html_e('All Severities', 'ai-seo-assistant'); ?></option>
                        <option value="critical"><?php esc_html_e('Critical', 'ai-seo-assistant'); ?></option>
                        <option value="high"><?php esc_html_e('High', 'ai-seo-assistant'); ?></option>
                        <option value="medium"><?php esc_html_e('Medium', 'ai-seo-assistant'); ?></option>
                        <option value="low"><?php esc_html_e('Low', 'ai-seo-assistant'); ?></option>
                    </select>
                    <select id="filter-status">
                        <option value="active"><?php esc_html_e('Active', 'ai-seo-assistant'); ?></option>
                        <option value="acknowledged"><?php esc_html_e('Acknowledged', 'ai-seo-assistant'); ?></option>
                        <option value="fixed"><?php esc_html_e('Fixed', 'ai-seo-assistant'); ?></option>
                    </select>
                    <button type="button" class="button" onclick="filterDecayAlerts()"><?php esc_html_e('Filter', 'ai-seo-assistant'); ?></button>
                </div>
            </div>
            ...
            <table class="wp-list-table widefat fixed striped">
                <thead>
                    <tr>
                        ...
                        <th><?php esc_html_e('Position Change', 'ai-seo-assistant'); ?></th>
                        <th><?php esc_html_e('Traffic Impact', 'ai-seo-assistant'); ?></th>
                        <th><?php esc_html_e('Severity', 'ai-seo-assistant'); ?></th>
                        <th><?php esc_html_e('Detected', 'ai-seo-assistant'); ?></th>
                ...
                <tbody>
                    ...
                    <tr data-severity="<?php echo esc_attr($alert['severity']); ?>" data-status="<?php echo esc_attr($alert['status']); ?>">
                        ...
                        <td>
                            <a href="<?php echo esc_url(get_edit_post_link($post->ID)); ?>" class="button button-small"><?php esc_html_e('Edit Post', 'ai-seo-assistant'); ?></a>
                            <button type="button" class="button button-small" onclick="showSuggestions(<?php echo esc_js($alert['id']); ?>)"><?php esc_html_e('Suggestions', 'ai-seo-assistant'); ?></button>
                            <button type="button" class="button button-small" onclick="acknowledgeAlert(<?php echo esc_js($alert['id']); ?>)"><?php esc_html_e('Ack', 'ai-seo-assistant'); ?></button>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                    ...
                    <tr>
                        <td colspan="7" style="text-align: center; padding: 40px;">
                            <p style="font-size: 16px; color: #666;">🎉 <?php esc_html_e('No content decay detected! Your content is performing well.', 'ai-seo-assistant'); ?></p>
                            <p class="description"><?php esc_html_e('We\'ll check daily and alert you when content starts losing rankings.', 'ai-seo-assistant'); ?></p>
                        </td>
                    </tr>
                    <?php endif; ?>
                </tbody>
        ...
        function acknowledgeAlert(id) {
            if (confirm('<?php esc_html_e('Acknowledge this alert? It will be moved to acknowledged status.', 'ai-seo-assistant'); ?>')) {
        ...
        function showSuggestions(id) {
            ...
            alert('<?php esc_html_e('Suggestions would appear in a modal here', 'ai-seo-assistant'); ?>');
        }
        </script>
        <?php
    }
