...
class AIRepurposer
{
    ...
    public function addMetaBoxes(): void
    {
        add_meta_box(
            'aiseo_repurpose',
            __('AI Content Repurposing', 'ai-seo-client'),
            [$this, 'renderMetaBox'],
            ['post', 'page'],
            'side',
            'default'
        ...
        jQuery(document).ready(function($) {
            $('.aiseo-repurpose-btn').on('click', function() {
                ...
                wp.apiFetch({
                    ...
                    btn.prop('disabled', false).text(btn.text().replace('<?php esc_html_e('Generating...', 'ai-seo-client'); ?>', ''));
                }).catch(function(error) {
                    alert('<?php esc_html_e('Error:', 'ai-seo-client'); ?> ' + error.message);
            ...
            $('#aiseo-copy-repurpose').on('click', function() {
                ...
                $(this).text('<?php esc_html_e('Copied!', 'ai-seo-client'); ?>').delay(2000).text('<?php esc_html_e('Copy', 'ai-seo-client'); ?>');
            });
        });
        </script>
        <?php
    ...
    public function repurpose(\WP_Post $post, string $format)
    {
        ...
        if (!isset($prompts[$format])) {
            return new \WP_Error('invalid_format', __('Invalid format specified', 'ai-seo-client'));
        }
