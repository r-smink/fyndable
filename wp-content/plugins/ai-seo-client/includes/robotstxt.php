...
class RobotsTxt
{
    ...
    public function renderSettings(): void
    {
        ...
        <p>
            <textarea name="aiseoclient[robots_txt_content]" rows="10" style="width:100%; font-family:monospace;"><?php echo esc_textarea($custom); ?></textarea>
        ...
        <div id="robots-rules">
            ...
            <div class="robot-rule" style="border:1px solid #ccc; padding:10px; margin:10px 0;">
                <p>
                    ...
                    <input type="text" name="aiseoclient[robots_rules][<?php echo $i; ?>][user_agent]" value="<?php echo esc_attr($rule['user_agent']); ?>" class="regular-text">
                </p>
                <p>
                    <label><?php esc_html_e('Allow paths (one per line)', 'ai-seo-client'); ?></label>
                    <textarea name="aiseoclient[robots_rules][<?php echo $i; ?>][allow]" rows="3" style="width:100%;"><?php echo esc_textarea(implode("\n", (array)$rule['allow'])); ?></textarea>
                </p>
                <p>
                    <label><?php esc_html_e('Disallow paths (one per line)', 'ai-seo-client'); ?></label>
                    <textarea name="aiseoclient[robots_rules][<?php echo $i; ?>][disallow]" rows="5" style="width:100%;"><?php echo esc_textarea(implode("\n", (array)$rule['disallow'])); ?></textarea>
                </p>
                <p>
                    <label><?php esc_html_e('Crawl-delay (seconds)', 'ai-seo-client'); ?></label>
                    <input type="number" name="aiseoclient[robots_rules][<?php echo $i; ?>][crawl_delay]" value="<?php echo esc_attr($rule['crawl_delay']); ?>" min="0" max="60">
                </p>
            </div>
            <?php endforeach; ?>
        </div>
